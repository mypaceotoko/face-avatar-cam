...
- 体や背景はそのままカメラ映像が見える
- 口の開閉、まばたき、眉、視線、頭の姿勢に追従(口パクは強調設定)
- グリーンバックモード:身体と顔(アバター付き)だけを切り抜き、背景をクロマキー緑で塗りつぶす
- ブラウザ内で録画 → ダウンロード可能
- Vite + React + TypeScript + Three.js + MediaPipe Tasks Vision (FaceLandmarker + ImageSegmenter)
- 外部の有料 API なし、Apple Memoji や VRM などの既存資産も使わずに、Three.js プリミティブだけで自作
...
4. **「グリーンバック ON」** をタップすると身体と顔だけが切り抜かれ、背景がクロマキー緑(#00B140)に置き換わります(初回のみセグメンテーションモデルの読み込みあり)
5. 「録画開始」→「録画停止」→ プレビューと「ダウンロード」が表示されます
6. 「Debug」をタップすると FPS と blendshape の値が見えます
